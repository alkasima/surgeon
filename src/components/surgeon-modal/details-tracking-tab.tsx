...
export function DetailsTrackingTab({ surgeon }: DetailsTrackingTabProps) {
  ...
  const handleSpecialtiesChange = (e: ChangeEvent<HTMLInputElement>) => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    toast({ title: "Details Updated", description: `${formData.name}'s information has been updated.` });
    setIsSaving(false);
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <Label htmlFor="name">Surgeon Name</Label>
          <Input id="name" name="name" value={formData.name || ''} onChange={handleChange} />
        </div>
        <div>
          <Label htmlFor="clinicName">Clinic Name</Label>
          <Input id="clinicName" name="clinicName" value={formData.clinicName || ''} onChange={handleChange} />
        </div>
        ...
        <div className="md:col-span-2">
          ...
          <Input
            id="specialties"
            name="specialties"
            placeholder="e.g., FUE, FUT, Beard Transplant"
            value={formData.specialties?.join(', ') || ''}
            onChange={handleSpecialtiesChange}
          />
           <p className="text-xs text-muted-foreground pt-1">Comma-separated values.</p>
        </div>
