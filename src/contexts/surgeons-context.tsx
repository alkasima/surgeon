...
export const SurgeonsProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    // This effect keeps the selectedSurgeon state in sync with the main surgeons list.
    // This is crucial for reflecting real-time updates in the modal (e.g., name change).
    if (selectedSurgeon) {
      const updatedSurgeonFromList = surgeons.find(s => s.id === selectedSurgeon.id);
      if (updatedSurgeonFromList) {
        // Simple stringify is cheap enough here to prevent re-renders on identical data.
        if (JSON.stringify(selectedSurgeon) !== JSON.stringify(updatedSurgeonFromList)) {
          setSelectedSurgeon(updatedSurgeonFromList);
        }
      } else {
        // The surgeon was deleted from the list, so close the modal.
        setIsModalOpen(false);
        setSelectedSurgeon(null);
      }
    }
  }, [surgeons, selectedSurgeon]);
